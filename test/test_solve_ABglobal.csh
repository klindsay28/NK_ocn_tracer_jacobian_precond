...
#BSUB -W 0:30                       # wall clock time (in minutes)
...
echo entering test_solve_ABglobal.csh
date

source ../scripts/shared_vars

setenv in_fname     $WDIR/fcn_eval_000.nc
setenv out_fname    /glade/scratch/klindsay/B_global.nc
...
mpirun.lsf job_memusage.exe ./bin/solve_ABglobal -D1 -n12,12 \
  -v IAGE_RESTORE_1DAY_CUR $matrix_fname $out_fname
set ret_code = $status
if ($ret_code) then
  echo solve_ABglobal returned with error code $ret_code
  echo exiting test_solve_ABglobal.csh
  date
  exit 1
endif
date
...
echo returning from test_solve_ABglobal.csh
date
